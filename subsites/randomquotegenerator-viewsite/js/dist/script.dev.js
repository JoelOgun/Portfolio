...
function getRandomQuote(array) {
  ...
  var randomNumber = Math.floor(Math.random() * array.length); // 2. Use the random number variable and bracket notation
  // to grab a random object from the `quotes` array, and
  // store it in a variable

  var grabObject = array[randomNumber]; // 3. Return the variable storing the random quote object
...
function getBackgroundColor(backgroundColor) {
  ...
  var randomColor = Math.floor(Math.random() * backgroundColor.length); // 2. Use the random number variable and bracket notation
  // to grab a random color from the `backgroundColor` array, and
  // store it in a variable

  var changeColor = backgroundColor[randomColor]; // 3. Return the variable storing the random background color
...
function printQuote() {
  ...
  var getFunctionRandom = getRandomQuote(quotes);
  var getFunctionColor = getBackgroundColor(backgroundColor); // 2. Create a variable that initiates your HTML string with
  ...
  var htmlP = " ";
  htmlP += '<p class = "quote">'
    .concat(getFunctionRandom.quote, '</p>\n            <p class = "source">')
    .concat(getFunctionRandom.source, "\n  "); // 3. Use an if statement to check if the citation property
  // exists, and if it does, concatenate a <span></span>
  // element, appropriate className, and citation property
  // to the HTML string

  if (getFunctionRandom.hasOwnProperty("citation")) {
    htmlP += '<span class="citation">'.concat(
      getFunctionRandom.citation,
      "</span>"
    );
  } else {
    htmlP;
  ...
  if (getFunctionRandom.hasOwnProperty("year")) {
    htmlP += '<span class="year">'.concat(getFunctionRandom.year, "</span>");
  } else {
    htmlP;
  } // 5. After the two if statements, concatenate the closing </p>
  // tag to the HTML string

  htmlP += "</p>"; // 6. set the innerHTML of the quote-box div to equal the
  // complete HTML string

  document.getElementById("quote-box").innerHTML = htmlP; // 7. style the background of a document to equal the random color function
